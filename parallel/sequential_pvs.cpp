...
void AIPlayer::sequential_pvs(GameBoard & gameBoard, int depth, Node & node, int alpha, int beta) {
	...
	if(node.type == type){ // MIN
		...
		for(int i = 0; i < n; ++i) {
			...
			if (i == 0) {
				sequential_pvs(gameBoard, depth - 1, subnodes[i], alpha, beta);
			} else {
				sequential_alphabeta(gameBoard, depth - 1, subnodes[i], beta - 1, beta);
				if (subnodes[i].value > alpha && subnodes[i].value < beta) {
					sequential_alphabeta(gameBoard, depth - 1, subnodes[i], alpha, beta);
				}
			}
			// traceback
		...
		for(int i = 1; i <= n; ++i) {
			...
			if(i == 1) {
				sequential_pvs(gameBoard, depth - 1, subnodes[size - i], alpha, beta);
			} else {
				sequential_alphabeta(gameBoard, depth - 1, subnodes[size - i], alpha, alpha + 1);
				if (subnodes[size - i].value > alpha && subnodes[size - i].value < beta) {
					sequential_alphabeta(gameBoard, depth - 1, subnodes[size - i], alpha, beta);
				}
			}
			// traceback
